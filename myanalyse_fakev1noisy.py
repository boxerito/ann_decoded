...
noise_scale = [0.05,0.1, 0.5, 1.0, 2.0, 5.0]
# noise_scale = [0.1]
...
for n_neurons in n_neu:
    ...
    for n_img in numbers_of_images:
        for noise in noise_scale:
            fname=f'neurons_to_cifar_{n_neurons}n_1rep{n_img}n_img_{noise}_noise'
            ...
            plt.ylim(0,0.25)
            np.save(f'ANNpictures/training_loss_{n_neurons}n_{n_img}img_{noise}noise', history.history['loss'])
            np.save(f'ANNpictures/validation_loss_{n_neurons}n_{n_img}img_{noise}noise', history.history['val_loss'])
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.legend()
            plt.title(f'Cost Function vs Epoch ({n_neurons} neurons, {n_img} images, {noise} noise)')
            #savefig
            plt.savefig(f'ANNpictures/cost_vs_epoch_{n_neurons}n_{n_img}img_{noise}noise.png')
            plt.show()
            ...
            for i in range(10):
                ...
                axs[1, i].axis('off')
            #save figure
            plt.title(f'Orig. vs Recons. ({n_neurons} n, {n_img} img, {noise} noise)',loc='right')
            plt.subplots_adjust(wspace=0, hspace=0.0)
            plt.savefig(f'ANNpictures/orig_vs_reconstructed_{n_neurons}n_{n_img}img_{noise}noise.png')
            plt.show()
            ...
            plt.subplots_adjust(wspace=0, hspace=0.0)
            plt.title(f'RFs ({n_neurons} neurons, {n_img} images, {noise} noise)',loc='right') 
            plt.savefig(f'ANNpictures/RFs_{n_neurons}n_{n_img}img_{noise}noise.png')
            plt.show()
    print(maxvalue)
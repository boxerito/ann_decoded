...
for n_neurons in n_neu:
    ...
    for n_img in numbers_of_images:
        for offset in offset_scale:
            fname=f'neurons_to_cifar_{n_neurons}n_1rep{n_img}n_img_{offset}offset'
            ...
            plt.ylim(0,0.06)
            np.save(f'ANNpictures/training_loss_{n_neurons}n_{n_img}img_{offset}offset', history.history['loss'])
            np.save(f'ANNpictures/validation_loss_{n_neurons}n_{n_img}img_{offset}offset', history.history['val_loss'])
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.legend()
            plt.title(f'Cost Function vs Epoch ({n_neurons} neurons, {n_img} images, {offset} offset)')
            #savefig
            plt.savefig(f'ANNpictures/cost_vs_epoch_{n_neurons}n_{n_img}img_{offset}offset.png')
            plt.show()
            ...
            for i in range(10):
                ...
                axs[1, i].axis('off')
            #save figure
            plt.title(f'Orig. vs Recons. ({n_neurons} n, {n_img} img, {offset} offset)',loc='right')
            plt.subplots_adjust(wspace=0, hspace=0.0)
            plt.savefig(f'ANNpictures/orig_vs_reconstructed_{n_neurons}n_{n_img}img_{offset}offset.png')
            plt.show()
            ...
            plt.subplots_adjust(wspace=0, hspace=0.0)
            plt.title(f'RFs ({n_neurons} neurons, {n_img} images, {offset} offset)',loc='right') 
            plt.savefig(f'ANNpictures/RFs_{n_neurons}n_{n_img}img_{offset}offset.png')
            plt.show()
    print(maxvalue)
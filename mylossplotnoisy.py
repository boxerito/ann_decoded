...
noise_list = [0.05,0.1, 0.5, 1.0, 2.0, 5.0]
plt.figure()
...
for n_img in img_list:
    for noise in noise_list:
        ...
        val_loss = np.load(f'ANNpictures/validation_loss_{n_neurons}n_{n_img}img_{noise}noise.npy')
        
        # Plot the validation loss
        plt.plot(val_loss, label=f'{n_img} img, {noise} noise')
...
plt.title('Validation Loss vs Epoch for Different noises and Images, 1000 neurons')
#savefig
plt.savefig(f'ANNpictures/validation_loss_{n_neurons}_noises.png',bbox_inches='tight') #ADD bbox_inches='tight' to savefig to avoid cropping the legend
...
for n_img in img_list:
    for noise in noise_list:
        ...
        train_loss = np.load(f'ANNpictures/training_loss_{n_neurons}n_{n_img}img_{noise}noise.npy')
        
        # Plot the training loss
        plt.plot(train_loss, label=f'{n_img} img, {noise} noise')
...
plt.title('Training Loss vs Epoch for Different noises and Images, 1000 neurons')
#savefig
plt.savefig(f'ANNpictures/training_loss_n_{n_neurons}_noises.png',bbox_inches='tight') #ADD bbox_inches='tight' to savefig to avoid cropping the legend
# Show the plot
plt.show()

# Initialize a 2D array to store the loss values
# Initialize a 2D array to store the loss values
loss_values_train = np.zeros((len(noise_list), len(img_list)))
loss_values_val = np.zeros((len(noise_list), len(img_list)))
...
axs[0].set_yticks(np.arange(len(noise_list)))
axs[0].set_xticklabels(img_list)
axs[0].set_yticklabels(noise_list)
axs[0].set_xlabel('Number of Training Examples')
axs[0].set_ylabel('Noise Level')
axs[0].set_title('Mean Training Loss (Noise level, Training Examples)')
...
axs[1].set_yticks(np.arange(len(noise_list)))
axs[1].set_xticklabels(img_list)
axs[1].set_yticklabels(noise_list)
axs[1].set_xlabel('Number of Validation Examples')
axs[1].set_ylabel('Noise Level')
axs[1].set_title('Mean Validation Loss (Noise level, Training Examples)')

# # Save the figure
plt.tight_layout()
fig.colorbar(im2, ax=axs, label='Mean Validation Loss', location='right')
plt.savefig(f'ANNpictures/mean_loss_noisy.png')

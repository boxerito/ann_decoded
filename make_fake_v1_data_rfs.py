...
class Neuron:
	...
	def __init__(self, width, height,offset):
		scale = np.random.random()*3 + offset
...
for offset_scale in offset_scale:
	neurons = []
	for _ in range(n_neurons):
		neurons.append(Neuron(width,height,offset_scale))
	...
	for n_img in numbers_of_images:
		...
		fname = 'neurons_to_cifar_' + str(n_neurons) + 'n_' + str(replicates) +'rep' + str(n_img) + 'n_img'+'_'+str(offset_scale)+'offset'
